...
let fuse = null; let currentQuery = "";
...
function buildFuse(items){
  ...
  fuse = new Fuse(items, {
    keys: [{name:'title',weight:0.45},{name:'artist',weight:0.45},{name:'genre',weight:0.06},{name:'label',weight:0.04},{name:'year',weight:0.04}],
...
function getCurrentIndex(){
  const sc = getScroller(); const center = sc.scrollLeft + sc.clientWidth/2;
  let bestI = 0, bestD = Infinity;
  getCards().forEach((el,i)=>{
    const rect = el.getBoundingClientRect();
    const left = rect.left + sc.scrollLeft - sc.getBoundingClientRect().left;
...
function render(){
  ...
  state.filtered.forEach((rec,i)=> scroller.appendChild(createCard(rec,i)));
...
function createCard(rec){
  ...
  inner.appendChild(sleeve); inner.appendChild(back);
...
const cacheKey = (r)=> `art:${(r.artist||'').toLowerCase()}|${(r.title||'').toLowerCase()}`;
...
async function attemptArtwork(rec, coverEl){
  ...
  for(const p of providers){ try{ const url = await p(rec); if(url){ applyCover(coverEl,url); rec.cover=url; cache.set(key,url); return; } }catch{} }
}
...
async function fetchMusicBrainzCover(rec){
  ...
  const res = await fetch(url, {headers:{'User-Agent':'VinylFlipbook/1.0'}}); if(!res.ok) return null;
  const data = await res.json(); const id = data['release-groups']?.[0]?.id; if(!id) return null;
  ...
  try{ const h = await fetch(art1200,{method:'HEAD'}); if(h.ok) return art1200; }catch{}
  try{ const h2 = await fetch(art500,{method:'HEAD'}); if(h2.ok) return art500; }catch{}
  return null;
}
...
function applySearch(q){
  ...
  if(parts.length>=2 && fuse){
    state.filtered = fuse.search(currentQuery).map(r=>r.item);
  }else{
...
async function importFile(file){
  ...
  if(ext==='json'){ const text = await file.text(); const parsed = JSON.parse(text); state.all = Array.isArray(parsed)?parsed:parsed.values||[]; build(); }
  else if(ext==='csv'){
    if(window.Papa){ Papa.parse(file,{header:true,skipEmptyLines:true,complete:(r)=>{state.all=r.data; build();}}); }
    else { const text = await file.text(); const lines=text.split(/\\r?\\n/).filter(Boolean); const headers=lines.shift().split(',').map(s=>s.trim()); const rows=lines.map(line=>{const cols=line.split(',');const o={};headers.forEach((h,i)=>o[h]=cols[i]);return o;}); state.all=rows; build(); }
  }
}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(menuToggle && controls){ menuToggle.addEventListener('click', ()=>{ const open=controls.classList.toggle('open'); menuToggle.setAttribute('aria-expanded', open?'true':'false'); }); }
  ...
  if(!state.all.length){ fetch('./vinyl_collection.cleaned.json').then(r=>r.json()).then(arr=>{ state.all=arr; build(); }).catch(()=> build()); } else build();
});
