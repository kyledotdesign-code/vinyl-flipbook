// VERSION: v32 — robust cover loader (sheet → sanitize → proxy → strict album lookup), remove Install button, add genre chips & Top Genres stats
'use strict';

// Turn this on to let the app fetch art *only if* your sheet's URL fails AND we find a strict match.
const SMART_FALLBACK = true; // iTunes/Deezer with strict match only
const USE_PROXY_ON_FAIL = true; // try images.weserv.nl if a host blocks hotlinking
...
function fourthColumnFallback(rec){
...
function mapRecord(r, i){
  ...
  let cover = pick(r,idx,['cover url','cover_url','cover','image','art','artwork']);
  if(!cover){ cover = fourthColumnFallback(r); }
...
function sanitizeCoverURL(u){
  ...
  m = url.match(/imgur\.com\/([A-Za-z0-9]+)$/);
  if(m && !/i\.imgur\.com/.test(url)){ return `https://i.imgur.com/${m[1]}.jpg`; }
  return url;
}
function proxyURL(u){
  try{
    const clean = u.replace(/^https?:\/\//,''); // weserv expects host/path without scheme
    return 'https://images.weserv.nl/?url=' + encodeURIComponent(clean) + '&w=1200&h=1200&fit=cover&we';
  }catch{ return u; }
}
...
async function attemptArtwork(rec, coverEl, force=false){
  ...
  if(!force){
    ...
    if(cached && await apply(cached)) return;
  }

  // 1) Sheet URL (sanitized)
  const fromSheet = sanitizeCoverURL(rec.cover||"");
  if(fromSheet){
    if(await apply(fromSheet)) return;
    // 1b) Try proxy if host blocks hotlink
    if(USE_PROXY_ON_FAIL && await apply(proxyURL(fromSheet))) return;
  }

  // 2) Smart strict fallback (Album-only, exact match artist+title)
  if(SMART_FALLBACK && hasMinInfo(rec)){
    let found = null;
    try{ found = await fetchITunesStrict(rec); }catch{}
    if(!found){ try{ found = await fetchDeezerStrict(rec); }catch{} }
    if(found){ await apply(found); return; }
  }
}

function applyCover(el, url){ if(!el) return; el.innerHTML=''; el.style.backgroundImage = "url('" + url + "')"; }

// iTunes strict (album entity, exact normalized compare)
async function itunesSearch(term, attribute){
  const q = encodeURIComponent(term); const attr = attribute ? `&attribute=${attribute}` : '';
  const url = `https://itunes.apple.com/search?media=music&entity=album&limit=50&term=${q}${attr}`;
  return withTimeout((ctl)=>fetchJson(url, ctl), 8000);
}
function strictOk(rec, cand){
  const ar1 = NOR_FLAT(rec.artist), ar2 = NOR_FLAT(cand.artistName);
  const t1  = NOR_FLAT(rec.title),  t2  = NOR_FLAT(cand.collectionName);
  return (ar1 && ar1===ar2) && (t1 && t1===t2);
}
async function fetchITunesStrict(rec){
  const tries = [
    await itunesSearch(`${rec.artist} ${rec.title}`, ''),
    await itunesSearch(`${rec.artist}`, 'artistTerm'),
    await itunesSearch(`${rec.title}`, 'albumTerm'),
  ];
  for(const d of tries){
    if(!d||!d.results||!d.results.length) continue;
    const exact = d.results.find(r => strictOk(rec, r) && r.artworkUrl100);
    if(exact){ return exact.artworkUrl100.replace('100x100bb', '1200x1200bb'); }
  }
  return null;
}
...
async function fetchDeezerStrict(rec){
  const q = encodeURIComponent(`artist:"${rec.artist}" album:"${rec.title}"`);
  const data = await deezerJSONP(`https://api.deezer.com/search/album?q=${q}`, 9000);
  const list = (data && data.data) || [];
  const exact = list.find(a => NOR_FLAT(a.artist && a.artist.name) === NOR_FLAT(rec.artist) &&
                               NOR_FLAT(a.title) === NOR_FLAT(rec.title));
  if(exact){ return exact.cover_xl || exact.cover_big || exact.cover_medium || null; }
  return null;
}
...
function lazyScheduleCover(coverEl, rec){
  const schedule = (force=false)=> loaderQ.push(()=> attemptArtwork(rec, coverEl, force));
...
function genreChips(rec){
  const g = splitGenres(rec.genre);
  if(!g.length) return null;
  const wrap = document.createElement('div');
  wrap.className = 'genres';
  g.slice(0,4).forEach(name=>{
...
function tile(rec, idx){
  ...
  if(rec.cover){ cover.style.backgroundImage="url('"+sanitizeCoverURL(rec.cover)+"')"; }
...
function splitGenres(s){
  return (s||'').split(/[\/,;&|•·]+|\s+\band\b\s+|\s*\+\s*/i).map(t=>t.trim()).filter(Boolean).map(x=>x.replace(/\b\w/g,m=>m.toUpperCase()));
}
function buildStats(items){
  ...
  for(const r of all){
    for(const g of splitGenres(r.genre)){
      gset.add(g);
  ...
  for(const r of all){ if(r.artist) artistCount.set(r.artist, (artistCount.get(r.artist)||0)+1); }
...
function openStats(){
  ...
  sum.innerHTML = ''
    ...
    + cardStat(s.totalGenres, 'Total genres');
  document.getElementById('topGenres').innerHTML = barsHtml(s.topGenres);
  document.getElementById('topArtists').innerHTML = barsHtml(s.topArtists);
...
async function refreshAllArt(){
  ...
  try{
    ...
    await Promise.all(covers.map(({el, rec})=> new Promise(resolve=>{
      loaderQ.push(async ()=>{
        await attemptArtwork(rec, el, true);
...
// ---------- PWA: SW (install UI removed) ----------
function registerSW(){
  if('serviceWorker' in navigator){
    navigator.serviceWorker.register('./service-worker.v32.js').catch(()=>{});
  }
}
...
(function(){
  ...
  onReady(async function(){
    ...
    // Nav buttons and wheel for flip view
