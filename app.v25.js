// VERSION: v25 — Better Refresh + optional Deep Refresh + Wikipedia fallback
'use strict';
...
async function attemptArtworkWithPreload(rec, coverEl, force=false){
  ...
  // Try iTunes first, then Wikipedia
  let found = null;
  try{ found = await fetchITunes(rec); }catch{}
  if(!found){
    try{ found = await fetchWikipedia(rec); }catch{}
  }
  if(!found) return;
...
// Wikipedia fallback (CORS-friendly via origin=*)
async function fetchWikipedia(rec){
  ...
  for(const q of queries){
    ...
    const search = await withTimeout((ctl)=>fetchJson(searchUrl, ctl), 8000);
    const pageid = search && search.query && search.query.search && search.query.search[0] && search.query.search[0].pageid;
    if(!pageid) continue;
    const pageUrl = `https://en.wikipedia.org/w/api.php?action=query&pageids=${pageid}&prop=pageimages&pithumbsize=1200&format=json&origin=*`;
    const page = await withTimeout((ctl)=>fetchJson(pageUrl, ctl), 8000);
    const pages = page && page.query && page.query.pages;
    const p = pages && pages[pageid];
...
async function refreshVisible(forceAll=false){
  ...
  try{
    ...
    // Clear computed covers unless user provided cover_url
