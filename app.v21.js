// VERSION: v21 — Google Sheets CSV fetch support
'use strict';
...
function render(){
  ...
  Array.from(root.querySelectorAll('.cover')).slice(0,12).forEach(el=>{ const c = el.closest('.tile').querySelector('.card'); attemptArtwork(c.__rec, el); });
  console.log(`Rendered ${state.filtered.length} records`);
}
...
async function fetchSheetCsv(){
  ...
  if(!url || /PASTE_GOOGLE_SHEET_CSV_URL_HERE/.test(url)) return false;
  try{
...
(function(){
  ...
  onReady(async function(){
    ...
    // Boot with fallback data, then try Google Sheet once.
    build();
    await fetchSheetCsv();
  });
})();