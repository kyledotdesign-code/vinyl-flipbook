// VERSION: v31 — adds Top Genres chart back; keeps summary lean; grid flips; covers from sheet; no iOS hint
'use strict';

const ALLOW_AUTO_LOOKUP = false; // set true to try iTunes/Deezer only if sheet has no cover
...
async function attemptArtworkWithPreload(rec, coverEl, force=false){
  ...
  // 1) Prefer cover from sheet
  if(rec.cover && !force){ if(await apply(rec.cover)) return; }
  ...
  // 3) Optional fallback
  if(ALLOW_AUTO_LOOKUP && hasMinInfo(rec)){
    let found = null;
...
function scoreCandidate(rec, cand){
  ...
  if(/remaster|deluxe|anniversary/.test(ct)) s-=0.3; return s;
}
async function fetchITunes(rec){
  ...
  for(const d of tries){
    ...
    if(best && best.artworkUrl100){
      return best.artworkUrl100.replace('100x100bb', '1200x1200bb');
    }
  }
  return null;
}
...
function splitGenres(s){
  return (s||'').split(/[\/,;&|•·]+|\s+\band\b\s+|\s*\+\s*/i).map(t=>t.trim()).filter(Boolean);
}
...
// ---------- PWA: SW + install (no iOS hint banner) ----------
function registerSW(){
